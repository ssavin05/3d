...
export const HERRAMIENTAS = {
  async buscar_espacios({ consulta = "", capacidad = null, tipo = null, fecha = null }) {
    ...
    let resultados = await buscar(consulta || tipo || "espacio", espacios.filter((e) => e.reservable), disponibilidad);
...
async function responderLocal(mensaje) {
  ...
  switch (intencion) {
    ...
    case "precio": {
      const espacios = (await api.getEspacios()).filter((e) => e.reservable && e.estado === "disponible");
    ...
    default: {
      ...
      const espacios = (await api.getEspacios()).filter((e) => e.reservable);
