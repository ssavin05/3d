...
export async function render(contenedor) {
  ...
  contenedor.addEventListener("click", async (e) => {
    ...
    if (mover) { ancla.setDate(ancla.getDate() + Number(mover.dataset.mover) * (vista === "mes" ? 28 : vista === "semana" ? 7 : 1)); return cargar(contenedor); }
    if (e.target.closest("[data-hoy]")) { ancla = new Date(); return cargar(contenedor); }
    const cambiar = e.target.closest("[data-vista]");
    if (cambiar) { vista = cambiar.dataset.vista; contenedor.querySelectorAll("[data-vista]").forEach((b) => b.setAttribute("aria-pressed", String(b === cambiar))); return pintar(contenedor); }
