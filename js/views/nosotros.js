...
async function cargarPrecioDesde(contenedor) {
  ...
  try {
    ...
    const precios = espacios
      .filter((e) => e.reservable && Number(e.precio_hora) > 0 && (e.portada || e.fotos?.some((f) => f?.url && !f?.es_360)))
...
async function cargarResenas(contenedor) {
  ...
  try {
    const espacios = (await api.getEspacios()).filter((e) => e.reservable && (e.portada || e.fotos?.some((f) => f?.url && !f?.es_360))).slice(0, 4);
