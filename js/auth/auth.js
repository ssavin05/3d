...
async function hidratarSesion(sesion) {
  ...
  // Membresías por organización (multiempresa).
  const { data: membresias } = await db.from("organizacion_usuarios")
    .select("*, organizaciones(id, nombre, slug)")
    .eq("usuario_id", sesion.user.id);
  if (membresias?.length) {
    store.set({ membresias }, "membresias");
    const mejor = membresias.find((m) => ["admin", "superadmin"].includes(m.rol))
      || membresias.find((m) => m.rol === "staff");
    if (mejor && !["admin", "superadmin"].includes(store.get().rol)) {
      store.set({ rol: mejor.rol }, "rol");
    }
  }
}
